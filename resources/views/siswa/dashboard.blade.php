<x-app-layout title="Dashboard Siswa">
    <x-slot name="header">
        <h2 class="text-lg font-black text-gray-800 tracking-tighter uppercase">Dashboard Siswa</h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
                <div>
                    <h3 class="text-3xl md:text-4xl font-black text-gray-900 tracking-tighter uppercase">
                        Dashboard <span class="text-purple-600">Belajar</span>
                    </h3>
                    <p class="text-xs font-bold text-gray-400 uppercase tracking-[0.3em] mt-2">
                        Selamat datang kembali di LMS SMK Ahmad Yani
                    </p>
                </div>
                ...
                <button @click="open = true" class="group bg-purple-600 hover:bg-purple-800 text-white px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-purple-100 transition-all flex items-center gap-3">
                    <i class="fas fa-plus-circle transition-transform group-hover:rotate-90"></i> Masuk Kelas
                </button>
                @endif
            </div>
                ...
                <div class="bg-white rounded-[3rem] overflow-hidden shadow-2xl transform transition-all sm:max-w-lg sm:w-full z-[1000] border border-purple-100 relative">
                    <form action="{{ route('kelas.join.proses') }}" method="POST">
                        ...
                        <div class="p-10 text-center">
                            ...
                            <h3 class="text-2xl font-black text-gray-900 mb-2 uppercase tracking-tighter">Masukkan Token</h3>
                            <p class="text-xs text-gray-400 mb-8 font-bold uppercase tracking-widest">Dapatkan kode 6 digit dari guru pengajar</p>
                        ...
                        <div class="bg-gray-50 px-10 py-8 flex justify-center gap-4">
                            ...
                            <button type="submit" class="bg-purple-600 text-white px-10 py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg shadow-purple-100 hover:bg-purple-800 transition-all">MASUK KELAS</button>
                        </div>
                    </form>
                </div>
            </div>
            ...
            <div class="space-y-6">
                <div class="flex items-center gap-3 ml-2">
                    ...
                    <h3 class="text-[10px] font-black text-gray-400 uppercase tracking-[0.3em]">Kelas Aktif Anda:</h3>
                </div>

                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                    @forelse($kelasSaya as $k)
                    <a href="{{ route('kelas.detail', $k->id) }}" class="group block">
                        <div class="bg-white p-8 shadow-sm rounded-[2.5rem] border border-gray-100 hover:shadow-2xl hover:border-purple-200 hover:-translate-y-2 transition-all duration-500 relative overflow-hidden h-full flex flex-col">
                            ...
                            <div class="absolute -right-6 -bottom-6 text-9xl text-gray-50 group-hover:text-purple-50 transition-colors duration-500">
                            ...
                            <div class="flex items-center justify-between pt-8 border-t border-gray-50 relative z-10 mt-8">
                                <span class="text-[10px] font-black text-purple-600 uppercase tracking-widest flex items-center gap-2">
                                    Mulai Belajar <i class="fas fa-arrow-right text-[8px] group-hover:translate-x-2 transition-transform"></i>
                                </span>
                            </div>
                        </div>
                    </a>
                    @empty
                        <div class="col-span-full bg-white p-20 text-center rounded-[3rem] border-2 border-dashed border-gray-100 flex flex-col items-center justify-center">
                            ...
                            <p class="text-gray-400 font-bold uppercase text-xs tracking-widest mb-2">Belum ada kelas diikuti</p>
                            <p class="text-[10px] text-gray-300 italic uppercase">Silakan klik tombol Masuk Kelas di atas untuk memulai</p>
                        </div>
                    @endforelse
                </div>
            </div>
